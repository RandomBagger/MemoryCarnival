...
(() => {
    ...
    function setStatus(text) {
        ...
        if (text)
            statusTimer = window.setTimeout(() => (statusBar.textContent = ""), 4000);
    }
    ...
    function mediaEl(item, forModal) {
        if (item.type === "audio") {
            if (!forModal) {
    ...
    function buildTile(folder, item) {
    ...
    function buildFolder(folder, data) {
        ...
        title.textContent = (_a = FOLDER_LABELS[folder]) !== null && _a !== void 0 ? _a : folder;
    ...
    async function buildAboutEditor() {
    ...
    async function render() {
        ...
        if (total === 0)
            setStatus("No assets yet. Drop some files in.");
    }
    void (async () => {
        main.innerHTML = "";
